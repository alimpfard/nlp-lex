...
#ifdef DEBUG_C_PLUS_PLUS_SERIALIZER
  std::cout << "write container<const T> " << v.t.size() << " elems"
            << std::endl;
#endif
  my_size_t sz = v.t.size();
  out << bits(sz);
...
static inline std::ostream &operator<<(std::ostream &out,
                                       Bits<C<T, N> &> const v) {
#ifdef DEBUG_C_PLUS_PLUS_SERIALIZER
  std::cout << "write array container<T> " << v.t.size() << " elems"
            << std::endl;
#endif
  my_size_t sz = v.t.size();
  out << bits(sz);
...
#ifdef DEBUG_C_PLUS_PLUS_SERIALIZER
  std::cout << "write array container<const T> " << v.t.size() << " elems"
            << std::endl;
#endif
  my_size_t sz = v.t.size();
  out << bits(sz);
...
template <template <class K, class T, class Hash = std::hash<K>,
...
static inline std::ostream &operator<<(std::ostream &out,
                                       Bits<M<K, V> &> const m) {
#ifdef DEBUG_C_PLUS_PLUS_SERIALIZER
  std::cout << "write unordered_map<K,V> " << m.t.size() << " elems"
            << std::endl;
#endif
  my_size_t sz = m.t.size();
  out << bits(sz);
...
static inline std::ostream &operator<<(std::ostream &out,
                                       Bits<M<K, const V> &> const m) {
#ifdef DEBUG_C_PLUS_PLUS_SERIALIZER
  std::cout << "write unordered_map<K,const V> " << m.t.size() << " elems"
            << std::endl;
#endif
  my_size_t sz = m.t.size();
  out << bits(sz);
